...
public final class CommUtil {
	...
	public static List<Map<String,Object>> getUrlResource(RequestMappingHandlerMapping requestMappingHandlerMapping){
		Assert.notNull(requestMappingHandlerMapping);
		...
		for(String str:classNameSet){
			...
			Iterator<Entry<RequestMappingInfo, HandlerMethod>>  ite1=set.iterator();
			while(ite1.hasNext()){
				Entry<RequestMappingInfo, HandlerMethod> entry=ite1.next();
				RequestMappingInfo info=entry.getKey();
				HandlerMethod method = entry.getValue();
				String className=method.getMethod().getDeclaringClass().getName();
				if(str.equals(className)){
					String url=getUrl(info.getPatternsCondition());
					String pageIndex="\\{pageIndex\\}";
					String pageSize="\\{pageSize\\}";
					url=url.replaceAll(pageIndex,"0");
					url=url.replaceAll(pageSize,"20");
					String removeText="\\{*\\}";
					Pattern p = Pattern.compile(removeText);
					...
					result.put("url", url);
					String type=getMethods(info.getMethodsCondition());
	...
	private static String getUrl(PatternsRequestCondition condition){
		StringBuilder sb=new StringBuilder();
		Set<String> set=condition.getPatterns();
	...
	private static String getMethods(RequestMethodsRequestCondition condition){
		StringBuilder sb=new StringBuilder();
		Set<RequestMethod> methodSet=condition.getMethods();			
		Iterator<RequestMethod> rm=methodSet.iterator();
		int i=0;
		while(rm.hasNext()){
			RequestMethod r=rm.next();
			String name=r.name();
			if(i==0){
				sb.append(name);
			}else{
